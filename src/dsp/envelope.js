 ...
 */
var Envelope = function(context, gate, levels, times, releaseStage,
                        onComplete) {
    Node.call(this, context, 1, 1);
    this.gate = new Parameter(this, 0, gate || 1);

    this.levels = [];
    for (var i=0; i<levels.length; i++) {
        this.levels.push(new Parameter(this, null, levels[i]));
    }

    this.times = [];
    for (var i=0; i<times.length; i++) {
        this.times.push(new Parameter(this, null, times[i]));
    }
...
Envelope.prototype = Object.create(Node.prototype);
